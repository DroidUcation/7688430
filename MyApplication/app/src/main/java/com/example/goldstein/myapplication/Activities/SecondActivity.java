...
public class SecondActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Cursor cursor = getContentResolver().query(CameraInfoContract.BASE_CONTENT_URI, projection, CameraInfoContract.CameraInfoEntry._ID+"=?", new String[]{"2"}, null);
        ...
        if(cursor != null) {
            cursor.moveToFirst();
            ((TextView) findViewById(R.id.as_text_view_content)).setText(cursor.getString(columnContentIdx));
            ((ImageView) findViewById(R.id.as_image_view)).setImageResource(cursor.getInt(columnImageIdx));
        }
