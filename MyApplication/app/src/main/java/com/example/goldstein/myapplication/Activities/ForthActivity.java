...
public class ForthActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Cursor cursor = getContentResolver().query(CameraInfoContract.BASE_CONTENT_URI, projection, CameraInfoContract.CameraInfoEntry._ID+"=?", new String[]{"4"}, null);
        ...
        if(cursor != null) {
            cursor.moveToFirst();
            ((TextView) findViewById(R.id.afo_text_view_content)).setText(cursor.getString(columnContentIdx));
            ((ImageView) findViewById(R.id.afo_image_view)).setImageResource(cursor.getInt(columnImageIdx));
        }
